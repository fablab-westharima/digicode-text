...
const M5_OK = ['m5stack_cores3', 'm5stamp_s3a', 'm5stack_atoms3', 'm5stack_atom_lite', 'm5stack_stickc_plus2']
  .map(id => ({ id, platform: 'esp32' }));
...
test('a board row applies to that board alone, while the other boards of its platform stay clear', () => {
  ...
  for (const board of M5_OK) assert.deepEqual(incompatFor(board), [findIncompat(MQTT, board)], board.id);
});
