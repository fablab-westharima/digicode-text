...
export const FLASH_GUIDES = {
  ...
  // XIAO ESP32C6 も USB がチップに直結（GPIO12/GPIO13 の内蔵 USB Serial/JTAG）。C5・S3 と同じで、
  // ダイアログに出る名前はこの板でまだ確かめていないので、何を選ぶかは手順文で言う。
  xiao_esp32c6: {
    steps: [
      { text: 'USB-CケーブルでボードをPCに接続する。', figure: 'usb-c-connect-xiao-c6' },
      { text: 'OKを押すと開くブラウザのポート選択ダイアログで、ボードを挿したときに増えたポートを選ぶ。', figure: 'port-dialog-usb-serial' },
    ],
    notes: ['書き込みの前後のリセットは自動で行われるので、ボタン操作は要らない。'],
  },
  ...
  // Stamp-P4 も USB がチップ直結（ESP32-P4 の GPIO24／GPIO25）。この板には押せるボタンが
  // 1 つも無く、EN も BOOT もスタンプ穴なので、手順は挿すだけ。入らなかったときに落とす
  // 2 つの穴は補足に置く。基板に文字の印字は無いので、図にも文字は書かない。
  m5stamp_p4: {
    steps: [
      { text: 'USB-CケーブルでボードをPCに接続する。', figure: 'usb-c-connect-stamp-p4' },
      { text: 'OKを押すと開くブラウザのポート選択ダイアログで、ボードを挿したときに増えたポートを選ぶ。', figure: 'port-dialog-usb-serial' },
    ],
    notes: [
      'このボードには押せるボタンが無いので、挿すだけでよい。書き込みの前後のリセットは、ESP32-P4の内蔵USBシリアルを通して行われる。',
      '書き込みモードに入らないときは、M5のPinMapでG35（BOOT）と書かれた穴をGNDに落としたままUSBを挿し、書き込みが始まってから外す。ENもG35も穴で、押すボタンにはなっていない。',
    ],
  },
