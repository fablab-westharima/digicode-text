...
import { BOARD_INFO, PRODUCT_INFO, RESPONSE_RULES, systemFor } from '../web/ai-context.js';
...
for (const [provider, model, api] of [['openai','gpt-5-mini','responses'], ['openai','gpt-4.1-mini','chat'], ['claude','claude-sonnet-5','messages']]) {
  test(`actual UI sends rules, product reference and snapshot through ${api}`, async ({ page, context }) => {
    ...
    const prompts = ['このコードを簡単に説明して','改善案だけ教えて。まだ変更しないで','このコードをこのアプリでビルドして書き込む手順を教えて'];
    // Exercise every selected board and both application settings without changing source/dependencies.
    const envs = ['xiao_esp32c3','pico','xiao_rp2040'];
