...
static const uint16_t IREG_ADC0 = 0;       // A0 生値
static const uint16_t IREG_ADC1 = 1;       // A0 をもう一度読んだ生値
static const uint16_t IREG_UPTIME = 2;     // 起動からの秒数
static const uint16_t COIL_LED = 0;

// この board が側面パッドに出している ADC は A0 = D0 = GPIO1 の 1 本だけ。A1..A4 の
// マクロは基板裏の JTAG パッド (GPIO2/3/4) と、Seeed のピン表に無い GPIO5 を指すので、
// 2 本目の入力レジスタも同じ A0 から埋める。
static const uint8_t ADC0_PIN = 1;   // XIAO ESP32C5 D0 = GPIO1 (A0)
// COIL_LED の出力先は基板上のユーザー LED (GPIO27)。LOW で点灯するアクティブ LOW。
...
void loop() {
  ...
  if (now - lastUpdate >= updateIntervalMs) {
    ...
    mb.Ireg(IREG_ADC1, (uint16_t)analogRead(ADC0_PIN));
