...
// ATOM Lite: この variant は A0 から An のマクロを定義しないので、ADC の GPIO 番号を直接書く。
// 外に出ている ADC1 は GPIO32 と GPIO33 の 2 本だけ。GPIO25 は ADC2 で、Wi-Fi 動作中はドライバ越しにしか読めない。
static const uint8_t ANALOG_INPUTS[3] = { 32, 33, 25 };
...
static Config config = { 2000, "atom-lite-01", false, 1.0f };
...
void setup() {
  ...
  applyConfig("{\"intervalMs\":1500,\"verbose\":true,\"scale\":1.02,\"nodeId\":\"bench-atom-lite\"}");
}
