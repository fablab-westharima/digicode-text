...
// Stamp-P4: この variant は A0 から A13 を定義しているが、どの GPIO かが分かるように番号を並べておく。
static const uint8_t ADC_PINS[3] = { 16, 17, 18 };
...
static void postTelemetry() {
  ...
  for (uint8_t i = 0; i < 3; i++) adc.add(analogRead(ADC_PINS[i]));
