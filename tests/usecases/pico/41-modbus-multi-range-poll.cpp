...
static const Range RANGES[] = {
  ...
  {"digital-in", KIND_DISCRETE, 0x0000, 16},
};
...
static uint8_t readRange(const Range& r) {
  switch (r.kind) {
    ...
    case KIND_COIL: return node.readCoils(r.start, r.count);
    default: return node.readDiscreteInputs(r.start, r.count);
  }
}
...
static void dumpRange(const Range& r) {
  ...
  Serial.print(':');
  // ビット系は 1 ワードに 16 点詰まって返るので語数が違う。
  const bool bitwise = (r.kind == KIND_COIL || r.kind == KIND_DISCRETE);
  const uint8_t words = bitwise ? (uint8_t)((r.count + 15) / 16) : r.count;
