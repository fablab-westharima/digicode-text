...
// @skip 対象外: NTPClient は UDP 実装を要求する。Raspberry Pi Pico には UDP を張る手段が無い
// @desc 参考: NTP での時刻取得は xiao_esp32c3 / wio_node の 56 / 57 / 58 で見る

#include <Arduino.h>

// NTPClient のコンストラクタは UDP& を取るだけなので、ダミーの UDP 実装を書けば
// 形式上は compile できてしまう。しかし Raspberry Pi Pico にはネットワークが無く、
...
void setup() {
  Serial.begin(115200);
  Serial.print("ntp is out of scope for pico; intended pool=");
