...
// @skip 対象外: Azure IoT Hub は TLS + MQTT 前提。Raspberry Pi Pico 単体ではネットワークに出られない
...
// 実時刻が要り、publish には TLS スタックが要る。Raspberry Pi Pico 単体ではどちらも無いため対象外。
//
...
void setup() {
  Serial.begin(115200);
  Serial.print("azure iot is out of scope for pico; intended hub=");
