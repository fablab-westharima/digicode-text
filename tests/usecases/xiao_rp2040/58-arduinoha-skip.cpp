...
// @skip 対象外: ArduinoHA は HAMqtt に Client& を渡す設計で、XIAO RP2040 には TCP を張る手段が無い
// @desc 参考: Home Assistant への自動登録は ESP 系 board のケース (52 / 53) で見る

#include <Arduino.h>

// HAMqtt(Client&, HADevice&) は Client 実装を受け取るだけなので、ダミーの Client を
// 書けば形式上は compile できてしまう。しかし XIAO RP2040 には無線も Ethernet も無く、
// 実機で接続できない構成を ok として残すことになるため対象外にする。
//
// discovery payload を組み立てる処理自体は 40-modbus-ha-discovery-json.cpp で
// Serial 出力として検証している。

static const char* INTENDED_DEVICE_ID = "digicode-xiao-rp2040";

void setup() {
  Serial.begin(115200);
  Serial.print("arduinoha is out of scope for xiao_rp2040; intended device id=");
