...
// @desc Modbus で読んだ値を ArduinoJson で telemetry JSON にし、Azure IoT Hub の publish topic へ流す
...
// 接続先も device key もダミー。実際の Hub へは繋がない。
#define AZURE_SDK_CLIENT_USER_AGENT "c%2F" AZ_SDK_VERSION_STRING "(ard;esp32c3)"
...
static const char* DEVICE_ID = "digicode-c3-modbus";
static const char* DEVICE_KEY = "ZGlnaWNvZGUtdGV4dC1kdW1teS1kZXZpY2Uta2V5MDI=";
...
static const int8_t RS485_RX = 4;
static const int8_t RS485_TX = 5;
static const uint8_t RS485_DE = 3;
...
static bool generateSasToken() {
  ...
  return !az_result_failed(az_iot_hub_client_sas_get_password(
...
static bool ensureConnected() {
  ...
  if (!generateSasToken()) {
    Serial.println("sas token failed");
    return false;
  }
  if (!mqtt.connect(mqttClientId, mqttUserName, sasToken)) {
    Serial.printf("iot hub connect failed state=%d\n", mqtt.state());
    return false;
  }
...
static bool buildTelemetry() {
  JsonDocument doc;
  doc["deviceId"] = DEVICE_ID;
  doc["seq"] = sequence++;
  doc["uptimeMs"] = millis();
  doc["epoch"] = (uint32_t)time(NULL);
  ...
  measurement["temperatureC"] = (int16_t)node.getResponseBuffer(0) / 10.0f;
  measurement["humidityPct"] = node.getResponseBuffer(1) / 10.0f;
  measurement["pressureHpa"] = node.getResponseBuffer(2);
  measurement["statusBits"] = node.getResponseBuffer(3);
...
void loop() {
  ...
  if (!mqtt.publish(telemetryTopic, payload)) Serial.println("publish failed");
  if (!ok) Serial.printf("modbus failures=%lu\n", (unsigned long)failures);
}
