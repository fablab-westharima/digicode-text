...
static bool ensureMqtt() {
  ...
  // ESP 系の ESP.getEfuseMac() にあたるのが arduino-pico では rp2040.getChipID()。
  // RP2040 のフラッシュ固有 ID から作られるので、板ごとに違う client id になる。
  String clientId = "pico-w-";
  clientId += rp2040.getChipID();
