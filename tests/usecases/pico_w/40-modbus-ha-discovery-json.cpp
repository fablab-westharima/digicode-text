...
// このケースは discovery payload の組み立てだけを見る。publish そのものは
// 31-mqtt-publish-skip.cpp と 58-arduinoha-skip.cpp で無線に載せて確かめるので、
// ここは上位のゲートウェイが Serial を読んでそのまま MQTT へ載せられる形
// (topic + payload の 2 行) で出す。
static const char* NODE_ID = "pico_w_rtu_01";
static const char* STATE_TOPIC = "digicode/pico_w_rtu_01/state";
...
static void emitDiscovery() {
  for (uint8_t i = 0; i < ENTITY_COUNT; i++) {
    ...
    device["model"] = "Raspberry Pi Pico W Modbus Gateway";
