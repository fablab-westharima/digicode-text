...
test('一覧は vendor の小見出しで括られ、vendor 名→ボード名の順。#env の option は /boards の順のまま', async ({ page, request }) => {
  ...
  expect(await page.locator('#board-list .board-item').allTextContents())
    .toEqual(['ESP32-C5-DevKitC-1', 'ESP32-DevKitC V4', 'M5StampC5', 'Raspberry Pi Pico', 'Raspberry Pi Pico W', 'Wio Node', 'XIAO ESP32C3', 'XIAO ESP32C5', 'XIAO ESP32S3', 'XIAO RP2040', 'ESPr Developer C5']);
