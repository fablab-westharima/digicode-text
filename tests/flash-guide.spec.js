...
test('XIAO ESP32C5の手順はこのボード専用の基板図で、S3やC3の図を使い回さない', async ({ page }) => {
  await ready(page, 'xiao_esp32c5');
  await page.click('#flash-guide-open');
  await expect(page.locator('#flash-guide-title')).toContainText('XIAO ESP32C5');
  await expect(page.locator('#flash-guide-steps')).toContainText('USB-C');
  expect(await page.locator('.flash-figure').evaluateAll(els => els.map(e => e.dataset.figure)))
...
test('M5StampC5の手順は挿すだけで、押すボタンが無いことを補足で言う', async ({ page }) => {
  await ready(page, 'm5stamp_c5');
  await page.click('#flash-guide-open');
  await expect(page.locator('#flash-guide-title')).toContainText('M5StampC5');
...
test('Pico Wの手順は無印Picoと同じ3手順で、基板の図だけPico Wのものになる', async ({ page }) => {
  ...
  await page.click('#flash-guide-open');
  await expect(page.locator('#flash-guide-title')).toContainText('Raspberry Pi Pico W');
  await expect(page.locator('#flash-guide-steps')).toContainText('BOOTSELボタン');
  await expect(page.locator('#flash-guide-steps')).toContainText('RPI-RP2');
  expect(await page.locator('.flash-figure').evaluateAll(els => els.map(e => e.dataset.figure)))
    .toEqual(['bootsel-hold-pico-w', 'drive-appear', 'drive-dialog']);
});
...
test('ESP32-DevKitC V4の手順はMicro-USBとポート選択の2枚で、ボタン操作は要らないと言う', async ({ page }) => {
  await ready(page, 'esp32_devkitc_v4');
  await page.click('#flash-guide-open');
  await expect(page.locator('#flash-guide-title')).toContainText('ESP32-DevKitC V4');
  await expect(page.locator('#flash-guide-steps')).toContainText('Micro-B');
  await expect(page.locator('#flash-guide-notes')).toContainText('ボタン操作は要らない');
  // 自動リセットが効かない個体への1行は手順ではなく補足に置く。
  await expect(page.locator('#flash-guide-notes')).toContainText('Bootを押したままENを押してから');
  expect(await page.locator('.flash-figure').evaluateAll(els => els.map(e => e.dataset.figure)))
    .toEqual(['micro-usb-connect-devkitc', 'port-dialog-usb-serial']);
});
...
const HELP_SECTIONS = [
  ...
  ['export', '持ち出し'], ['ai', 'AI'], ['compiler', 'compile サーバー'], ['trouble', '困ったとき'],
  ['about', 'このソフトについて'],
];

test('取説は9節を目次で出し分け、対応ボードはボード表から出て、接続手順を開ける', async ({ page, request }) => {
  const boards = await (await request.get('/boards')).json();
  await ready(page, 'pico');
  await page.click('#view-help');
  await expect(page.locator('#help-dialog')).toBeVisible();

  // 目次はこの9節、この順、この文言。見出し（h3）も同じ文言。
