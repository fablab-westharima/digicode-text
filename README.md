...
- **PlatformIO プロジェクト**: `compiler/pio-rp2040`(`xiao_rp2040`・`pico`・`pico_w` の 3 env)、`compiler/pio-esp32`(`esp32_devkitc_v4`)、`compiler/pio-esp32s3`(`xiao_esp32s3`)、`compiler/pio-esp32c3`、`compiler/pio-esp8266`。チップの系統ごとに 1 ディレクトリで、platform の指定はその中で閉じる(教訓 1)。ESP 系が共有する build スクリプトは `compiler/pio-esp/`(`portable_paths.py`、`package_firmware.py`)。global な lib_deps は持たない。
