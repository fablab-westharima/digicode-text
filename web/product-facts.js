...
export const UI_FACTS = [
  ...
  { ids: ['view-settings', 'ai-key', 'ai-delete'], text: '左のアクティビティバーの「設定」を押すと、設定の画面が編集画面の上に開く。左の目次に「外観」「AI API設定」の節があり、「AI API設定」にAPIキーの欄がある。キーの保存先は利用者のブラウザだけで、「保存」の節の「キー・保存設定を削除（即時）」で消せる。' },
