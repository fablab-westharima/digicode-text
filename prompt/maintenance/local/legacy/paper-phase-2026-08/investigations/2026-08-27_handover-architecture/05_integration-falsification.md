...
latest observed tree での内訳は **UPHELD 2 / OVERSTATED 3 / REFUTED 3 / UNVERIFIABLE 0**。C4・C7 は支持された。C2 は約 33% という規模こそ近似したが、旧値は同じ推定器で **88,138 estimated tokens**、latest は「disk 再読込をしない」という新契約を守っても **60,299 estimated tokens**で、rule 13 は今も disk 再読込を命じている。後から出現した in-scope `04_cold-start-reconstruction.md` も読み、17/17 reconstruction・2 false facts・6/6 negative-control detection を本報告の独自 mutation と照合した。

この lane 中に integration owner が並行訂正し、router/body は 46→47→48 rows、read-load は 58,262→58,761→59,570 estimated tokens、brief は 88,618→90,100→93,501 bytes、`CLAUDE.md` は 10,060→10,236→10,848 estimated tokensへ動いた。stub 53、B70/B71 attribution、GEN close contract、baton 52、context-brief の不存在 guard claim は訂正されたが、最後の測定中にも DT-8 と baton 55 が追加され、tree は quiescent にならなかった。その更新で C1/C3/C8 の exact claims は latest tree と一致しなくなった。**このまま Human へ出すのは不適合**である。

## C1 — unconditional cold-start read

**Verdict: REFUTED**

packet 開始 tree の `bash scripts/read-load.sh` は RC=0、**58,262 estimated tokens / allowance 48,000 estimated tokens = 121% / inputs 7/7**で、claim はその snapshot では再現した。しかし integration owner の並行訂正後、latest run は RC=0、**59,570 / 48,000 = 124% / inputs 7/7**。`git archive HEAD` の throwaway tree は RC=0、**65,868 / 48,000 = 137% / inputs 7/7**だった。`scripts/read-load.sh` 自体に working-tree diff はなく、全 run は header の同じ式

```text
tokens = ascii_chars / 2.862 + nonascii_units / 0.788
```

を使用する。latest corrected statement は **65,868→59,570 estimated tokens、137%→124%、7/7 inputs、差 -6,298 estimated tokens**。これは architecture split だけの効果ではなく、`CLAUDE.md` 増加、S008 Human rulings、hook/guard/case 訂正も含む。

## C2 — real cold-start context cost / duplicate read

**Verdict: OVERSTATED**

S007 baseline `99d629c` と S008 integration tree それぞれで hook を実行し、JSON の `additionalContext` を取り出し、`read-load.sh` header と同じ式を独立に適用した。
...
| latest `read-load.sh` total | 59,570 estimated tokens |
| latest whole hook context | 23,260 estimated tokens |
| latest handover full | 22,130 estimated tokens |
| latest hook framing beyond full handover + bug index | 729 estimated tokens |
...
**Corrected statement:** 「同じ estimator による規範上の worst/careful-reader model は旧約 88,138、新契約を一貫して守る場合は latest 約 60,299 estimated tokens（約 -31.6%）。ただし rule 13 と read contract が衝突しており、二重読込解消は未成立。87,781 は hook framing を除いた部分値である。」

## C3 — no current truth deleted / verbatim relocation

**Verdict: REFUTED**

packet 開始時の `bash scripts/handover-diff.sh` は RC=0、**3 owners / 84 previous / 134 current / REWORDED 1 / GONE 0**だった。しかし latest tree は RC=0、**3 owners / total 84→138 / REWORDED 1 / GONE 1**。GONE は並行訂正で全面更新された旧 baton 52 body を指す。したがって Human-facing current claim の `GONE 0` は literal に反証された。integration owner が `done / relocated / dropped by user / lost` のいずれかへ分類する必要がある。
...
**Corrected statement:** 「旧 baton 45/45 と evidence block の全行は static comparison で残り、§4 も 1 行の参照明確化を除き保持された。その後の correction を含む latest `handover-diff.sh` は GONE 1/84 を示すため分類が必要。いずれにせよ 3-owner current truth 全体の non-deletion detector ではない。」

## C4 — every stub carries the protection needed for safety

**Verdict: UPHELD**

latest router/body を全 **48/48 baton IDs** で照合した。lane C が semantic dependency を判定したのは 45 batons（2–52）であり、31 `SAFE_ONLY_WITH_STUB` の integrator 版 stub は、実測更新された 25/43 を除き lane C の minimum payload と一致する。14 `SAFE_IF_TRIGGERED` rows も、少なくとも未採用・未検証・Human GO・誤 proxy 等の limitation を router row 内に保持する。
...
integration owner は lane 中に stub 53 を拡張し、ROSTER・brief allowlist・hook manifest・CLAUDE §0・handover-diff OWNERS の同一 commit 更新を router に入れた。新設 54 は detector limitations、新設 55 は B54 が §2/§3 しか走査しない limitation と executable guard 不在を stub に保持する。latest tree では、lane C 後に追加された 53–55 を含め、body を開かない reader に必要な prohibition/limitation が router にある。
...
しかし 77,190 snapshot は uncommitted intermediate state で repository に保存されておらず、同一 tree を独立再実行できない。`git archive 99d629c` の旧 exporter は別 temp root / transcript corpus という confounderで 77,354 bytes・RC=3となり、比較値として無効だった。また 11,428-byte の net growth には loop/queue だけでなく、S008 product rulings、`CLAUDE.md` 増分、owner export shape、dirty-tree metadata/digest が同時に入る。「増えたから completeness が増えた」という一方向の因果は instrument が分離していない。

さらに `04_cold-start-reconstruction.md` と本 report が現れた tree では **90,100 bytes**、DT-8 / baton 55 追加後の latest run は RC=0、**93,501 bytes**へ動いた。88,618 は current value ではなく integration時点の snapshot である。

**Corrected statement:** 「integration claim 時点 88,618 bytes、latest observed tree 93,501 bytesで、以前 omit していた loop/feedback queue を現に含む。lane B が earlier snapshot を 77,190 bytes と記録したが、delta は複数変更と metadata を含み、全増分を completeness に帰属できない。」
...
同一 `read-load.sh` の file rowで S007 baseline `99d629c` の `CLAUDE.md` は **8,871 estimated tokens**。packet 開始時は **10,060、差 +1,189**で claim を再現したが、parallel corrections 後の S008 commit `a8d5946` は **10,848、差 +1,977 estimated tokens**。Human に出る current cost は +1,189 ではない。

**Corrected statement:** 「S008 `a8d5946` の `CLAUDE.md` cost は S007 `99d629c` 比 +1,977 estimated tokens。増分の一部は後述の current-fact ownership violation であり、cost の開示は配置を正当化しない。」

## C1–C8 外で見つかった defects

### 🔴 ADJACENT_DEFECT — `CLAUDE.md` が current fact を所有し、すでに drift した

`CLAUDE.md:57-73` は「この file は current fact の owner ではない」「measured count を書くな」と禁止するが、§0 `:21-28` は **2026-08-27 S008 / owner set=3 / baton count / B69-B71** という current architecture を所有する。latest enumeration は **48/48**。`:21` は「45 は past measurement、live count は B71」と訂正されたが、`:25` はなお **“body and grounds of all 45 batons”** と current count を断言する。selftest B54 は `CLAUDE.md` §§2–3 しか scan せず、§0 に移した同型 violation を green にした。baton 55 はこの穴を記録したが executable guard は未実装。
...
packet は「measured number を update trigger のない file に書くこと」を明示的に禁じた。`scripts/context-brief.sh:24-25` と baton 52 body は **77,190→90,100 bytes / 91%** を prose にコピーしたが、latest output はすでに **93,501 bytes**。script comment に再測定/更新 trigger はなく、baton body の trigger は cap proximity/別 objective で毎回の output変化ではない。C6 が示した volatility を同じ形式で永続化し、同 lane 内で stale になった。

### 🟢 RESOLVED-DURING-LANE — parallel integrator corrections

packet-start で見つけた `/close` GEN 自己矛盾、baton 52 stale body、stub 53 の欠落、context-brief comment の不存在 guard claim、B70/B71 attribution は integration owner が後続 tree で訂正した。これは本 lane が直したものではない。訂正後の再測定が C1/C3/C4/C6/C8 の latest verdict に反映されている。

### 🔴 ADJACENT_DEFECT / CONFLICT_SURFACE — falsification 中に tree が凍結されなかった

Lane E の測定中に integration owner が owner files、`CLAUDE.md`、scripts、さらに `prompt/maintenance/global/rules/common/judgment-mistakes-history.md` の DT-8 を並行編集した。DT-8 自身が **「反証レーンを走らせている間はツリーを凍結する」**と防御に書く一方、その case と baton 55 の追加で Lane E の測定はもう一度 stale になった。read-load は 58,262→58,761→59,570、brief は 88,618→90,100→93,501、owner entries は 134→136→138。この report は latest observed snapshot を記録するが、quiescent final tree の証拠ではない。なお Lane E 自身は packet の禁止どおり `prompt/maintenance/global/` を変更していない。

### 🟡 ADJACENT_DEFECT / CONFLICT_SURFACE — draft report を含む integration commit が lane 終了前に作られた

最終確認時、HEAD は S008 integration commit `a8d5946` へ進んでおり、commit 自身がこの `05_integration-falsification.md` の未完成 draft を含んでいた。本 lane の以後の訂正は同 file の working-tree diff として残る。integration owner が commit owner であるため Lane E の `Do NOT commit` 違反ではないが、falsification の verdict 確定前に対象 claim と report を同じ commit に固定したため、packet の「before those claims are put to the Human」と tree-freeze contract を満たす順序ではない。

### 🟢 HARDENING — Human が列挙した forbidden surfaces は保持

`git diff --name-only 99d629c..a8d5946` と threshold compare では、application `package.json` / `tsconfig.json` は双方 absent、`Project_Template` diff は 0 files、`READ_ALLOWANCE=48,000 tokens`、`READ_REVIEW=68,000 tokens`、`BRIEF_MAX_BYTES=98,304 bytes` は baseline と同一。Human が明示した product code、template、threshold の禁止は保持された。current truth の actual static comparisonでも旧 baton ID の欠落はなかった。なお integration commit は global case history 1 file に DT-8 を追加しており、「global diff 0」ではない。本 packet は Lane E に global write を禁じるが、integration owner の DT-8 追加を禁止する authority までは示さないため、これは上記の concurrent-edit conflict として記録し、scope 違反とは断定しない。
...
**現状のまま Human に提出するのは不適合。** 数値改善の方向と latest C4/C7 は立つが、C1/C3/C8 の exact claims は parallel correction 後の latest tree と一致せず、C2 は conflicting read contracts を解消した前提でも約 60.3k、旧 exact valueも 357 estimated tokens 過少である。C5 の checks は architecture を壊す現実的 mutation を targeted green にし、C6 の self-measuring byte value は update trigger なしで source/body へ再転記され同 lane 内で stale になった。tree を凍結して final read-load/brief/CLAUDE cost を再測定し、GONE 1 を分類し、cold-start contract と guard scope を限定するまでは「real cost」「GONE 0」「detection power」を Human-facing claim に残せない。

## Verification record

- `[synthetic]` `bash scripts/read-load.sh; RC=$?` → packet-start 58,262/48,000 (121%)、中間 58,761/48,000 (122%)、latest 59,570/48,000 estimated tokens (124%)、全て 7/7・RC=0。
- `[synthetic]` S007 `99d629c` archive 内 `bash scripts/read-load.sh; RC=$?` → 65,868/48,000 estimated tokens, 137%, 7/7, RC=0。
- `[synthetic]` old/current hook JSON + repository estimator の独立再計算 → old hook 22,671 / clipped 21,914 / framing 356; latest hook 23,260 / handover 22,130 / framing 729 estimated tokens; probe RC=0。
- `[synthetic]` `bash scripts/handover-diff.sh; RC=$?` → packet-start owners 3・84→134・GONE 0、中間 84→136・GONE 1、latest 84→138・GONE 1、全て RC=0。
- `[synthetic]` `bash scripts/context-brief.sh > "$brief_out"; RC=$?; wc -c` → 88,618→90,100→93,501 bytes、全て RC=0。rulings 3/3 と loop/queue を payload で確認。
- `[synthetic]` last full `bash scripts/selftest.sh; RC=$?` → 78 passed / 0 failed、B71 47/47、RC=0。その後 baton 55 が加わったため latest 48/48 は static enumerationで、full selftest は tree freeze 後に再実行が必要。
- `[static]` `99d629c` / S008 exact-row compare → old baton 45/45 present, 44/45 exact; evidence block 46/46 exact; §4 19/20 exact lines; probe RC=0。
...
- `[static]` `git diff --name-only 99d629c..a8d5946` + threshold/scope compare → 3 thresholds unchanged; Project_Template diff 0 files; global diff 1 file; package.json/tsconfig.json 0/2 present。
