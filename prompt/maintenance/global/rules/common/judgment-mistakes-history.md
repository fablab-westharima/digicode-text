...
| DT-1 | digicode-text S001 (2026-08-26) | case 16 family, **restrictive direction** + rule 17 inverse | a donor asset's PROPRIETARY marking written up as "can never be ported", without ever asking who holds the rights |
| DT-2 | digicode-text S003 (2026-08-26) | PT-4/83/85 family, **directional-error version** | 9 probe defects in one session; every one made the subject look worse, and four had reached draft findings |
...
### case DT-1 — a licence marking read as a portability verdict (2026-08-26, S001)

- **Class**: **case 16 family** (a licence decided by convention instead of by reading), in the **restrictive direction**; plus **rule 17 inverse** (self-imposed scope, exclusion side) and Pattern B (self-confirmed scope). Neighbours: PT-8 (rule 17 inverse, inclusion side), case 16 (three libraries assumed MIT were GPL / AGPL / dual).
- **Situation**: the DigiCode donor audit found `digicode-class-server`'s README marked **PROPRIETARY / All rights reserved** — the only asset in the ecosystem not marked AGPL-3.0. That fact was correct and was measured. What was written from it was not: the audit deliverable and the handover both recorded 「digicode-text (public / AGPL-3.0) へは一切持ち込めない」 at 🔴, and the handover row argued the point further — *「製品判断は覆せてもライセンス境界は覆せない」*. The user overturned it the same day: **the rights-holder is the same company**, a marking records the terms the code is *currently offered* under, and the rights-holder may offer the same code to digicode-text under different terms. `PROPRIETARY = 移植不可` as a mechanical rule is now explicitly forbidden.
- **Root cause**: a **marking** was consumed as a **constraint**. The question "may this be ported" is about **rights**, and rights were never asked about — not the holder, not whether third-party code sits inside, not what terms this project could itself offer. Case 16 is the identical failure with the sign flipped: there, "major lib = MIT" was assumed permissive; here, "marked proprietary" was assumed prohibitive. Both skipped the same step, which is reading who owns what.
- **Why it survived my own review**: because it was the **conservative** error. A restrictive mistake feels like caution, so it does not trip the check that a permissive one would. It was even written up as a *strength* — filed as a separate 🔴 row precisely *because* it looked like a boundary that outranked product judgment. **The confidence came from the direction of the error, not from evidence.**
- **The second defect inside the first**: the conclusion removed an option from the user's decision space and stated it as immovable. That is rule 17's shape on the exclusion side — scope narrowed by the harness, not by the user — and it was harder to see than usual because it arrived dressed as legal rigour rather than as 「out of scope」.
- **Defense**:
  1. **A licence marking answers "under what terms is this currently offered", never "may we use this".** Before writing any portability or adoption verdict, name the **rights-holder** first. If you cannot name one, the verdict is `追加調査必要`, not a boundary.
  2. **Separate five questions and never collapse them**: who holds the rights · is third-party code inside · what terms bind any bundled / copied / dependency code · what the source project currently offers it under · what this project could offer it under. **The last two are different questions**, and conflating them is what produced this case.
  3. **Give a conservative conclusion the same scrutiny as a permissive one.** 「安全側に倒した」 is not evidence; on this case it is the whole reason the error reached a persisted record.
  4. **Never write 「製品として不要」 and 「license 上不可」 in the same breath.** One is revisable by the user at any time; the other is a legal fact. Mixing them hides which one is actually load-bearing.
- **Scope**: every adoption / portability judgment; any audit that issues a verdict from **metadata** (a marking, a filename, a badge, a header) rather than from the thing the verdict is about; both directions of a licence call.
- **Related**: case 16 · `17-no-self-imposed-scope.md` (exclusion side) · `24-objective-control.md` · PT-8 · `11-dependency-upgrade.md`.
- **Retroactive application, same session (case-filing protocol)**: re-checked this session's other verdicts for the same shape — a conclusion drawn from metadata rather than from the thing itself. Two were re-examined and stand on read evidence rather than markings: 「`arduino-compile-server` は廃止済み」 rests on the successor's README stating the decommission date, not on the directory's absence; 「Compile API は任意ソースを受け取れない」 rests on `inject.ts`'s substitution code, not on the endpoint's name. The third-party OSS licences in `09_editor-lsp-survey.md` are already labelled **二次情報** with 「本文は読んでいない」 stated in the file, so they claim no more than they measured.

### case DT-2 — 9 probe defects in one session, and every one of them made the *subject* look worse (2026-08-26, S003)

- **Class**: **PT-4 / 83 / 85 family** (the instrument is silently wrong), with a property those cases do not name: **the error had a direction**. Neighbours: PT-28 (the verifier's own input was malformed), PT-30 (the reader of the instrument was wrong), case 82 (a pipe hid the exit code), case 85 (one poll reused as a synchronization point).
- **Situation**: two consecutive isolated-probe objectives (Editor / LSP Technical Spike, then Local LSP Helper Feasibility) produced **9 defects in my own measurement harness**, all found and corrected inside the same session (`investigations/2026-08-26_editor-lsp-spike/01` §6 P-1〜P-5, `investigations/2026-08-26_local-helper-feasibility/01` §5 Q-1〜Q-4). Four of them had already been written into draft findings before being caught:
  - **P-3**: a previous probe's `arduino-language-server` process was still alive when the next probe started. `documentSymbol` / `completion` / `definition` all timed out. The draft read **「ALS は references も documentSymbol も返さない」**. Run alone, `documentSymbol` returned `["lastPublishMs","setup","loop","reportBoot"]` immediately.
  - **P-5**: after sending `didChange`, the completion position was computed from the **pre-edit** text. The server correctly answered `-32602 non-added document`. The draft read **「`.ino` は編集するたびに補完が壊れる」**. With the position computed from the post-edit text, completion returned the right cross-file members.
  - **Q-2**: the helper was stopped with `pkill -f "helper/helper.js"`, which did not match a process started as `node helper.js`. **Phase 1 — "helper absent", the central claim of the entire objective — ran with the helper alive** and reported `helper: "connected"`. Nothing about the fallback had been demonstrated.
  - **Q-3**: a shell loop building an Origin truth table mangled its quoting and sent a malformed header. The helper returned **400**, and the draft read **「Origin 検証が 403 を返さない」**. A single verbose request returned **403**, as designed.
- **Root cause**: in every case the harness **failed to establish a precondition it had assumed**, and the assumption was invisible because *the subject answered*. A dead process, a stale position, a live leftover, a broken header — each produced a plausible negative from a real system, which is indistinguishable from a real negative unless the precondition is measured separately.
- **🔴 The property worth the case number**: **all four failures pointed the same way — they made the subject look worse than it is.** That is not chance. A probe that under-supplies its subject (wrong position, stale state, malformed request, contended resource) yields *failures*; a probe that over-supplies it does not silently yield *successes*, because success requires the subject to actually do the work. **So instrument defects are systematically biased toward false negatives, and false negatives read as findings.** Two of these were headed for a Human Architecture Decision as reasons to down-rank an OSS candidate.
- **Why the existing defenses did not fire**: rule 04 §absence says to check the positive control before trusting a count — but its subject is *the acceptance criterion*, and here the absence was in the **setup**, not the criterion. The harness had positive controls for what it was measuring (SAB present/absent, permission granted/denied) and **none for whether the world was in the state the measurement assumed**.
- **Defense**:
  1. **Measure the precondition, do not arrange it.** Before a phase that depends on a state ("the helper is not running", "no other instance is up", "the port is free"), **assert that state with a command and record the result as a datum**. The scenario probe was only trustworthy after `phase1_precondition_port_free = true` appeared *in the output*. Arranging a precondition and assuming it took is exactly the gap.
  2. **Kill by the resource, not by the name.** `pkill -f <pattern>` matches a command line, which is a description of how something was started, not of what it holds. Kill by the port / the pid file / the lock — the thing whose absence you are about to claim.
  3. **A negative result about someone else's software is a claim about your harness until you have reproduced it in isolation.** Before writing "X does not support Y", run Y **alone**, in a clean process, with the narrowest possible request. Three of this session's four false negatives died on the first isolated re-run.
  4. **Recompute every position and identifier from post-mutation state.** Any offset, line, column or index derived before an edit is stale by construction; the server's rejection is then correct and reads as a defect.
  5. **When a negative finding is about to be written, ask which direction the instrument's error would point.** If a harness bug would produce *this same* result, the finding is unverified — regardless of how many times it reproduced under the same harness.
- **Scope**: every isolated probe; every comparative evaluation of external software; every multi-phase scenario where an earlier phase's teardown is a later phase's precondition; every measurement whose result will enter a Human decision as evidence about a third party.
- **Related**: PT-4 · PT-28 · PT-30 · case 82 · case 83 · case 85 · `04-testing-strategy.md` §When the acceptance criterion is an absence (this case is the **setup-side** neighbour of that section) · `04-testing-strategy.md` §The instrument must measure the dimension you are judging.
- **Retroactive application, same session (case-filing protocol)**: re-checked every negative finding in both objectives for "would a harness bug produce this?". Survivors, each re-verified: **`referencesProvider: ABSENT`** is read from the server's own `initialize` response, not inferred from a timeout — it is the server declaring the gap. **`panic: not implemented`** was isolated by a two-arm control (with / without `hierarchicalDocumentSymbolSupport`) and the panic text came from the subject's own stderr. **The permission block** carries a negative control on the other arm and the helper log confirms nothing arrived. **Raw clangd failing on `.ino`** is a one-process, one-request measurement with the driver's own error text. **The 500 MiB/session figure** was re-measured after isolating clangd-build from board as the variable. No negative finding survived that could not name evidence independent of my harness's assumptions.
- **Forward application (2026-08-16 clause — apply to the verifications still ahead)**: the close that follows this filing re-runs its gates on the **final** tree and takes each RC on its own line, because "the gate was green earlier" is the same shape — a claim about a tree that no longer exists.
- **Same-day recurrence (S004, hours after filing — the strongest evidence this case has)**: the handover-maintenance objective that followed wrote a pointer-integrity checker that resolved bare filenames against a fixed list of base directories. It reported **9 unresolved pointers** in a file whose pointers were all valid. The reds were produced by the checker under-supplying itself with base paths — **defense 5's exact question ("would a harness bug produce this same result?") answers yes** — and a second run with the correct bases returned **0 unresolved**. Filing the case did not prevent the recurrence; **running its own defense did**. That is the finding: this case's value is not that it is recorded, it is that defense 5 is asked before a negative is written down.

### case DT-3 — a mandatory consult trigger fired for five sessions and never once fired (2026-08-26, S006)

- **Class**: **PT-11 / PT-14 / PT-36 family** (the trigger never reaches the moment it governs), crossed with **PT-6's author's version** (a rule enumerated, then not applied by the person who read it). Neighbours: PT-13 (the anti-forgetting anchor sat unfilled for four sessions), case 18 (a trap recognized and the recommendation kept).
- **Situation**: S001–S005 ran with `codex tool calls = 0`, `subagent spawns = 0`, five sessions in a row. S006 was opened by the Human specifically to audit why. The audit — deliberately delegated to Codex as a `FALSIFICATION` lane, because the parent auditing its own operation is case PT-2's shape — produced three findings, of which **only one could not be refuted**:
  - `22-model-orchestration.md:421` states, without condition: *"Consult the different-vendor lane before concluding when … (b) **considering a direction change**."* S005 (Product Value Revalidation) re-examined from zero **why the product should exist at all**, and its output became the Human rulings that redefined the product. The trigger applied. It did not fire.
  - The recorded justification for solo across S002/S003 was *「delegate へ渡すと evidence が claim に劣化する(rule 04)」* — a **misreading**. `04-testing-strategy.md:311-326` defines an E1/E2/E3 evidence contract precisely so that delegated work yields evidence, and states *"Routine parent per-claim reproduction is forbidden too"*: the rule pushes work **away from** the parent. The proposition "delegating degrades evidence into a claim" is `NOT FOUND` in rules 04 and 22 (full-text search, `prompt/` 108 files).
  - Of six sessions, **only S002 recorded a route line at the moment work started**. S001 and S005 have none at all; S000/S003/S004 carry one only in the close-time summary — the exact form rule 22:609-614 names as insufficient (*"the close-time mix can only show that no delegation happened, never whether the option was weighed at the moment it mattered"*).
- **Root cause**: three layers, and only the third is about judgment.
  1. **The trigger has no enumerated antecedent.** rule 22 itself writes that a described trigger does not fire — the reader "follows edges, fires only on enumerated antecedents". §Independent-perspective triggers says *"considering a direction change"* and never defines or enumerates what that is, and nothing in the harness watches for it.
  2. **The default leans solo and is meant to.** `PRIMARY_MODEL_MODE` undeclared = `T1-solo`, whose row reads *"The normal state: solo execution is the sanctioned default"*. That is by design; what it does not license is skipping the route line or the mandatory consult.
  3. **A misread rule was reused as precedent.** Written once in S002, it was copied forward into S003 and (in the close aggregate) S005. **No session re-opened rule 04 to check it.** A justification, once written into a session file, reads as settled.
- **🔴 The part worth the case number**: **the cause was not in this repository's rules at all, and looking only at the rules would have closed the investigation with the wrong answer.** The S006 session's own system prompt carried, from the Claude Code product and not from any file in `.claude/settings.json`, `~/.claude/settings.json`, `~/.claude/CLAUDE.md` or `~/.claude/output-styles/`: *"Do not call the AgentTool unless the user requested it."* That instruction contradicts rule 22's default and **is the direct cause of `subagent spawns = 0`** — while explaining nothing about `codex tool calls = 0`, because it does not cover the MCP lane. A governance repository can be internally consistent and still be overridden by the harness hosting it.
- **The audit corrected the auditor.** The parent stated four conclusions and had Codex attack them; **three were partially refuted** — "no rule conditions delegation on a user instruction" (rule 12:261,265 does gate delegated *implementation* on a GO), "all four sessions used the same reason" (S004's reason was different and correct; S005 recorded none), and "the structure made delegation impossible" (`routing-scenarios.py:275` resolves an empty scale to `effort=NONE` = supported + BASELINE, so **baseline dispatch was always permitted** — the structural excuse was false). Only the mandatory-consult finding survived.
- **Defense**:
  1. **A justification written into a session file is a claim, not a precedent.** Before reusing a prior session's reason for not doing something, **open the rule it cites and read the clause**. Cost: one file read. This one was reused across three sessions and was wrong from the first writing.
  2. **Write the route line before the work, not at close.** rule 22 already requires this; what this case adds is that the close-time record is *indistinguishable* between "considered and declined" and "never considered", and the audit needs the difference.
  3. **When a rule says a consult is mandatory and names no enumerated antecedent, the antecedent is yours to write down at the moment you notice it applies.** "Am I re-examining what this product is for?" is a direction change. So is "should we keep building this?".
  4. **When the rules cannot explain an observed behaviour, look outside the rules before concluding the behaviour was a judgment failure.** The harness's own injected instructions, the model's configuration, and the transport's capability are all upstream of the rule text and none of them are in the repository.
  5. **Audit your own operation from another lineage.** The parent's self-audit was 3-for-4 wrong in the direction of overstating its own case; the delegated falsification is what caught it.
- **Scope**: every session that opens without a declared route; every reuse of a prior session's stated reason; every mandatory-consult clause whose antecedent is described rather than enumerated; every investigation into why a harness behaved as it did.
- **Related**: PT-11 · PT-13 · PT-14 · PT-36 · PT-6 · case 18 · `22-model-orchestration.md` §Independent-perspective triggers / §Routes / §Session mode · `04-testing-strategy.md` §Delegated verification claims · `13-session-recovery.md` §Step 4.5 (an instruction handed to a session is a claim about state).
- **Template feedback candidates (this repository does not edit `Project_Template`)**: (a) the start-of-work route line has **no runtime audit** — selftest B26 checks for the presence of nine strings in rule/close text, which cannot distinguish a route line written at 09:00 from one written at close; (b) §Independent-perspective triggers has **no firing mechanism and no enumeration**.
- **Retroactive application, same session (case-filing protocol)**: re-checked this session's own delegation record for the shape "a reason written once and reused". Six packets were dispatched, each with its lane chosen from the work rather than from a template, and `EFFORT_REASON: BASELINE` was written explicitly in all six rather than omitted — which is what rule 22:835 asks for, so that "the author forgot" and "the author chose baseline" do not collapse into the same record. The one reused justification in this session is `ROUTE_EFFORT: NONE`, and it was re-derived from `routing-profile.md` at dispatch time rather than carried from a prior session, because no prior session had ever dispatched.
- **Forward application (2026-08-16 clause)**: the close that follows this filing writes its own route/mode/lane mix into the session file **and** states the eligible-task denominator, because a mix without a denominator is the close-time record this case just called insufficient.

### case DT-4 — the conclusion was contradicted by its own investigation's most-emphasized finding, eleven files earlier (2026-08-26, S006 auditing S005)

- **Class**: **case 59 family** ("does not exist" derived from too narrow a search), with the search collapsed to a single document set — and **case 21 / PT-15 family** in that the unsupported claim was written into a persisted record carrying its strongest evidence label. Neighbours: pattern C (a sample treated as a whole population), PT-37 (a display read back as data).
- **Situation**: S005's `08_conclusion-and-next.md:48` lists the **first** ground for the product's Go verdict as 「権限ゼロの PC で書き込みまで成立する選択肢が**他に存在しない**(Arduino Cloud は Agent 必須)」, typed **`primary source`**. Eleven files earlier, the same investigation's `02_existing-environments-2026.md:100-113` describes Codey Online as 「**ローカルインストール不要**」 plus 「ブラウザ(Chrome / Edge)から **Web Serial で直接書き込み**」, also typed **`primary source`**, under a heading that reads **「これは本調査で見つかった最も重い事実である」**. The two statements are directly contradictory, both carry the strongest evidence label the project uses, and both were written by the same actor in the same session.
- **Discovery**: a delegated `FALSIFICATION` lane returned it as `CONFLICT_SURFACE`; the parent then opened both files and read the lines. An independent web lane, run without being shown the other lane's answer, additionally found a **second** product (PleaseDontCode — 35+ boards, AI that compiles firmware, USB/WiFi upload, free tier) that the S005 investigation never mentions at any point. So 「他に存在しない」 fails twice: against the investigation's own §5-1, and against a product it never found.
- **Root cause**: **the conclusion file was written as a summary of the investigation's *thesis*, not as a reconciliation against its *findings*.** Nothing in the process required the author to walk the conclusion's claims back through the evidence files. Section §5-1 was not buried — it was flagged as the single most important thing found — and it still did not reach a document written days-equivalent later in the same session, because no step ever asked "does any file in this directory contradict this line?".
- **🔴 The property worth the case number**: **case 59's failure was a search that was too narrow; this one's search had already succeeded.** The contradicting evidence was found, correctly typed, marked as the heaviest fact in the investigation, and written down — and the conclusion contradicted it anyway. That makes the defense different: widening the search would not have helped. What was missing was a **reconciliation pass against the author's own corpus**. An investigation large enough to need eleven files is large enough that its author no longer holds it all.
- **Defense**:
  1. **Before writing a conclusion file, grep your own investigation for each load-bearing claim's negation.** A claim of the form "no X exists" is checked against the directory that was supposed to look for X. This is a mechanical step and takes minutes.
  2. **Any claim of non-existence carries its denominator and its search record** — what was searched, how, and what was excluded and why. S005's comparison set was five units with no recorded selection rule, no search expressions, no geography and no candidate universe; that is unreportable as an absence regardless of what it found.
  3. **A `primary source` label describes where a fact came from, never whether the sentence built on it is true.** Two contradictory sentences here both carried it. Type labels grade provenance; they do not grade inference.
  4. **When a file says "this is the most important thing found", that sentence is a pointer into every later document.** Treat a self-declared heaviest finding as an obligatory input to the conclusion, not as one section among many.
  5. **A conclusion the Human will rule on gets a different-lineage falsification pass before it is submitted** — which is also case DT-3's finding, from the other direction.
- **Impact separation (rule: Human rulings are not overturned by a finding)**: the ruling (S005 §1/§2/§3 — core value, one-sentence definition, Web-version value), the original evidence (no zero-privilege write alternative; no product selling a guaranteed environment), the corrected evidence (two such products exist, primary-source confirmed; the absence claim has no denominator), the impact (the *empirical* basis of the core-value axis is gone; the axis itself may still be right), and the question for the Human (re-rule / keep-and-replace-the-grounds / keep-and-record-the-limit) are recorded separately in `investigations/2026-08-26_orchestration-re-audit/08_conclusion-and-next.md` §3. **The audit did not touch the ruling.**
- **Scope**: every multi-file investigation that ends in a conclusion document; every non-existence claim; every summary written from a thesis rather than from a reconciliation; any objective whose output feeds a Human ruling.
- **Related**: case 59 · case 21 · PT-15 · PT-37 · pattern C · `04-testing-strategy.md` §When the acceptance criterion is an absence · `01-investigation.md`.
- **Retroactive application, same session (case-filing protocol)**: applied the defense to this session's own conclusion file before committing it. `08_conclusion-and-next.md` §2-5 was added as a result — it records what **survived** the audit (V7, E3, A4, E1's bundle ratio) at the same strength as what was refuted, because a conclusion assembled only from the refutations would have been the same failure with the sign flipped. Each of the nine ruling/evidence rows in §3 was walked back to the file that owns its evidence, and the two rows whose corrected evidence *strengthens* the original ruling (QA counts, non-registry breakdown) are labelled as such rather than dropped for being inconvenient to the narrative.

### case DT-5 — a whole objective's evidence existed only in the transcript until the user asked whether the session had been closed (2026-08-26, S006)

- **Class**: **PT-21 family** (uncommitted evidence with no artifact behind it) crossed with **PT-36** (the discipline existed and its only trigger was the close boundary). Neighbour: `13-session-recovery.md` §Step 4.6 / `reference/context-handoff.md`, whose whole subject is this failure.
- **Situation**: S006 ran six delegated lanes and audited 82 claims. Every one of the five prior objectives (S001–S005) had written its measurements into `local/investigations/2026-08-26_*/` **as it produced them**. S006 wrote **nothing** into the repository for the entire objective. At the moment the objective was reported complete and the session stopped, the audit's results existed in exactly three places: the conversation transcript, a Markdown file the user had asked for in `~/Downloads`, and a published artifact — **none of them in git, and none of them reachable by a cold start.** 16.md still read `GEN: S005-close` and contained no mention that a re-audit had happened. A cold start would not have been able to discover that the audit existed at all.
- **It was not caught by a check. It was caught by the user asking** 「セッションクローズ処理終わってるんだっけ？」. Had the session ended there, the objective's entire output would have been lost from the project.
- **Root cause**: **the parent treated "write the evidence down" as part of close rather than as part of the work.** rule 13 §Step 4.6 says the opposite in as many words — *"write load-bearing state to disk as it is produced, not at close"* — and its stated reason is exactly this scenario. The reason it did not fire is structural: **the parent produced no evidence with its own hands.** Every measurement arrived as a delegate's returned message, and a returned message does not feel like a file being written. The five prior objectives had the parent running the probes, so the artifacts landed on disk as a by-product of doing the work; delegation removed that by-product and nothing replaced it.
- **🔴 The property worth the case number**: **delegation silently removes the incidental persistence that solo work provides.** The orchestration rules cover who may do what, what a packet must contain, and how a result is reviewed — and say nothing about where the returned evidence is written down. A `RESULT CAPSULE` is a message. The more successfully an objective is delegated, the less of it exists on disk, and the failure is invisible because every individual step was done correctly.
- **Defense**:
  1. **Create the objective's evidence directory at dispatch time, not at close.** The first delegation of an objective is the trigger: before sending packet #1, `00_index.md` exists and names the lanes.
  2. **A returned `RESULT CAPSULE` is written to disk in the same turn it is read.** Not summarized — the capsule and its claims index, verbatim enough that a later session can audit the parent's summary against it. This session's `03`–`07` files were reconstructed from the transcript at close; had the context been compacted first, they could not have been.
  3. **The close protocol's baseline check should be able to notice this.** An objective that produced no `investigations/` entry, in a project where every prior objective produced one, is a detectable anomaly — and here it was found by a human question instead.
  4. **Do not treat "the report was delivered to the user" as persistence.** A file in `~/Downloads` and a published artifact are outside git by design in this project (baton 16), so neither is a record the project owns.
- **Scope**: every delegated objective; every session whose deliverable is evidence rather than code; any objective where the parent's own tool calls are governance rather than measurement.
